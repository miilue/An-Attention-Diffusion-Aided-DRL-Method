...
from .service_provider import EdgeServer
...
class SwarmManager:
    ...
    def __init__(self, if_test):
        ...
        self._n_edge_servers = NUM_EDGE_SERVER
        self._users = [User(uid) for uid in range(self._n_users)]
        self._service_providers = [EdgeServer(eid) for eid in range(self._n_edge_servers)]
        self._task_generator = TaskGenerator(if_test)
    ...
    def assign(self, action, curr_time):
        assert self._querying_user, "No querying user found, call next_user_task first"

        if type(action) is np.ndarray:
            assert len(action) == 1
            action = int(action.item())

        eid = action // NUM_SERVICE_PROVIDERS
        sid = action % NUM_SERVICE_PROVIDERS

        service_provider = self._service_providers[eid]
        service_provider.assign_sid = sid
        assert service_provider.id == eid, \
            f"Service provider (id={service_provider.id}) not match assigned eid={eid}"
    ...
    def total_t_available(self):
        return sum([service_provider.total_t for service_provider in self._service_providers])

    @property
    def total_t_serving(self):
        return sum([service_provider.used_t_sum for service_provider in self._service_providers])

    @property
    def most_availble_service_provider(self):
        return np.argmax([service_provider.available_t for service_provider in self._service_providers]) * NUM_SERVICE_PROVIDERS

    @property
    def best_reward_service_provider(self):
        best_reward_ = 0.
        best_sid_ = 0
        for edge_server in self._service_providers:
            if edge_server.is_enough(self._querying_user.task):
                for service_provider in edge_server._service_providers:
                    reward_ = service_provider.calculate_reward(self._querying_user.task)
                    if reward_ > best_reward_:
                        best_reward_ = reward_
                        best_sid_ = edge_server.id * NUM_SERVICE_PROVIDERS + service_provider.id
        return best_sid_
    ...
    def monitor(self):
        ...
        for service_provider in self._service_providers:
            infos = service_provider.info
            for info in infos:
                print(f"{str(info['eid']).center(6)}"  
                      f"{str(info['id']).center(6)}" 
        ...
        print(f"\033[0;31mCRASHED: {total_crashed} " 
              ...
              .center(43), end='')
        print(f"\033[0;32mFINISHED: {total_finished} "  # 完成任务数总和
