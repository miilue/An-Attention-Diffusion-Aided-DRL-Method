...
class Task:
    def __init__(self, task_id, task_type, arrival_time, t):
    ...
    @property
    def task_type(self):
        return self._task_type.typeid

    @property
    def task_typecode(self):
    ...
    @property
    def vector(self):
        assert self._task_type is not None, f"Please set task type for task {self._task_id} first"
        vec = np.hstack([self._task_type.one_hot, self.norm_t, self.norm_runtime])
        return vec

    def can_finished(self, curr_time):
        assert not self._crashed, f"This task {self._task_id} has been crashed"
...
class TaskGenerator:
    ...
    def reset(self):
        self._task_id_counter = 0
        if self.if_test == True:
            np.random.seed(SEED)
        self._total_task = np.random.poisson(self._lambda * self._total_time)
        if self.if_test == True:
            print('total_task: ', self._total_task)
        self._task_arrival_time = np.hstack([[0], np.sort(np.random.random(self._total_task) * self._total_time)])
    ...
    def __next__(self):
        ...
        task_type = np.random.randint(low=0, high=NUM_TASK_TYPES)

        arrival_time = self._task_arrival_time[task_id]
        required_t = np.random.choice(self._t_range)

        task = Task(task_id, task_type, arrival_time, required_t)
