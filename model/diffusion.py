...
class Diffusion(nn.Module):
    def __init__(self, state_dim, action_dim, model, max_action,
                 ...
                 loss_type='l1', clip_denoised=False, predict_epsilon=True, device='cpu'):
        super(Diffusion, self).__init__()
        self.device = device
        self.preprocess = model
        self.output_dim = int(np.prod(action_dim))
    ...
    def p_mean_variance(self, x, t, s):
        x_recon = self.predict_start_from_noise(x, t=t, noise=self.preprocess(x, t, s))
        ...
        return model_mean, posterior_variance, posterior_log_variance  # μ, Σ, log(Σ)
    ...
    def p_sample_loop(self, state, shape, verbose=False, return_diffusion=False):
        ...
        for i in reversed(range(0, self.n_timesteps)):
            ...
            x = self.p_sample(x, timesteps, state)  # p(x_t-1 | x_t)
            progress.update({'t': i})
    ...
    def p_losses(self, x_start, state, t, weights=1.0):
        ...
        x_recon = self.preprocess(x_noisy, t, state)
